...
export class ReservationsService {
  ...
  async remove(id: string) {
    // IMPORTANT: capture vehicleId BEFORE deletion — after delete it's gone
    const r = await this.reservationModel.findById(id).lean().exec();
    if (!r) throw new NotFoundException('Reservation not found');
    const vehicleId = r.vehicle?.toString();

    await this.reservationModel.findByIdAndDelete(id);

    // Recompute vehicle status now that this reservation is gone
    if (vehicleId) await this.vehiclesService.syncVehicleStatus(vehicleId);
