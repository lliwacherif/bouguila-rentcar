...
// ════════════════════════════════════════════════════════
function StatusModal({ reservation, allReservations = [], onClose, onSaved }) {
  ...
  return (
    <div className="vm-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      ...
      >
            ...
            <div>
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--white)', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                <span style={{
                  ...
                  #TCR-{reservation._id?.slice(-6).toUpperCase()}
                </span>
              </div>
        ...
        {/* Tabs */}
        <div style={{ display: 'flex', borderBottom: '1px solid var(--black-4)', padding: '0 24px' }}>
          {TABS.map(([k, l]) => (
            <button
              key={k}
          ...
          <div className="admin-card">
            <div className="admin-card__header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
              ...
              <div style={{ position: 'relative', width: 320, maxWidth: '100%' }}>
                <input
                  type="text"
                  placeholder="🔍 Code (#TCR-...), Client, Email, Voiture..."
              ...
              <table className="admin-table">
                ...
                <tbody>
                  ...
                  {filteredReservations.map(r => {
                    const code = `#TCR-${r._id?.slice(-6).toUpperCase()}`
                    return (
                      <tr key={r._id} className="admin-table__row">
                        <td>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      {activeTab === 'parcs' && (
        <div className="admin-full">
          ...
          <div className="parc-board">
            {/* Unassigned column */}
            <div
              className={`parc-col parc-col--unassigned ${dragOverParc === 'unassigned' ? 'parc-col--drag-over' : ''}`}
            ...
            {parcs.map(parc => (
              <div
                key={parc._id}
                className={`parc-col ${dragOverParc === parc._id ? 'parc-col--drag-over' : ''}`}
